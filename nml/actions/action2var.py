    ...
    def preprocess_ternaryop(self, expr):
        ...
        if not expr.expr1.is_read_only() or not expr.expr2.is_read_only():
            return create_ternary_action(
                expr.guard, expr.expr1, expr.expr2, self.extra_actions, self.action_feature, self.var_scope
            )
...
def create_ternary_action(guard, expr_true, expr_false, action_list, feature, var_scope):
    act6 = action6.Action6()

    var_range = 0x8A if get_scope(feature, 0x8A) == var_scope else 0x89
